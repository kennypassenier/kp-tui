...
3. **Where else the same fault sits.** The fault is "a screen compared
   ...
   ticked off". It sits in four screens, not one: the footers of deploy
   (scroll, answer, background), settings (value, add tier, edit webhook,
   save) and splash (any key) promise keys that do nothing, mostly because
   a demo-wide key (`a`, `s`, Esc, Enter) catches them first.
...
| fix-65-M1 | Does a rebuilt screen still do everything the original did? Measured at this commit (2026-09-19): the two log tests are green, and the log screen binds all five keys homelab binds plus the level filter it lacks. Again at the sixth rebuilt screen. Measured 2026-09-26 at doctor: failed — its key handler was not read, enter did nothing, and the sweep that followed found the same on deploy, settings and splash. Fallback applied: the key inventory in `docs/HOMELAB_PROOF.md` [fix-68]. | failed → fix-68 |
...
| fix-68-M1 | Is a rebuilt or changed screen's key inventory filled before it is called done? Measured at this commit (2026-09-26): the doctor test is green, and the inventory covers the global keys and all eight rebuilt screens, with every ✗ row named. Again at the next screen rebuilt or changed. | open |
